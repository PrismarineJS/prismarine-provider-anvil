...
class RegionFile {
  constructor (path) {
    ...
    this.chunkTimestamps = undefined
    this.sectorFree = undefined
    this.sizeDelta = undefined
    this.lastModified = 0
    this.ini = undefined
  ...
  async _initialize () {
    ...
    for (let i = 0; i < RegionFile.SECTOR_INTS; ++i) {
      ...
      if (offset && ((offset >> 8) + (offset & 0xFF)) <= this.sectorFree.length) {
  ...
  async _write (x, z, nbtData) {
    ...
    if (sectorNumber && sectorsAllocated === sectorsNeeded) {
